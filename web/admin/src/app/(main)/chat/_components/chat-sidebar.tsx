  ...
  return (
    ...
    >
      <SidebarContent>
          ...
          <SidebarMenu className="gap-1">
            {navItems.map((item) => (
              <SidebarMenuItem key={item.id}>
                <SidebarMenuButton className="[&_svg]:size-3.5" size="sm" isActive={item.isActive} tooltip={t(`nav.${item.id}`)}>
                  <item.icon />
        ...
        <SidebarGroup>
          <SidebarGroupLabel className="font-normal">{t("channels")}</SidebarGroupLabel>
          <SidebarMenu className="gap-1">
            {channelItems.map((item) => (
              <SidebarMenuItem key={item.id}>
                <SidebarMenuButton className="[&_svg]:size-3.5" size="sm" isActive={item.isActive} tooltip={t(`channels.${item.id}`)}>
                  {item.id in channelBrandIcons ? (
                    <SimpleIcon icon={channelBrandIcons[item.id as keyof typeof channelBrandIcons]} />
                  ) : (
                    <item.icon />
                  )}
                  <span className="font-medium">{t(`channels.${item.id}`)}</span>
                </SidebarMenuButton>
                {item.label && <SidebarMenuBadge className="font-medium">{item.label}</SidebarMenuBadge>}
              </SidebarMenuItem>
            ))}
        ...
        <SidebarGroup>
          <SidebarGroupLabel className="font-normal">{t("views")}</SidebarGroupLabel>
          <SidebarMenu className="gap-1">
            {viewItems.map((item) => (
              <SidebarMenuItem key={item.id}>
                <SidebarMenuButton className="[&_svg]:size-3.5" size="sm" isActive={item.isActive} tooltip={t(`views.${item.id}`)}>
                  <item.icon />
                  <span className="font-medium">{t(`views.${item.id}`)}</span>
                </SidebarMenuButton>
                {item.label && <SidebarMenuBadge className="font-medium">{item.label}</SidebarMenuBadge>}
              </SidebarMenuItem>
            ))}
