      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between gap-4 px-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon-sm" aria-label={t("tag")}>
                  <Tag />
                </Button>
              </TooltipTrigger>
              <TooltipContent>{t("tag")}</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
              ...
              <TooltipContent>{t("snooze")}</TooltipContent>
            </Tooltip>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-44">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuGroup>
                  <DropdownMenuItem variant="destructive">{t("block_contact")}</DropdownMenuItem>
                </DropdownMenuGroup>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
          ...
          <MessageScrollerViewport>
            <MessageScrollerContent className="gap-6 px-2 py-8">
              <Marker variant="separator">
                <MarkerContent>{t("date_separator")}</MarkerContent>
              </Marker>
                      ...
                      <MessageContent>
                        <BubbleGroup>
                          <Bubble variant={isOutbound ? "default" : "muted"} align={message.align}>
                            ...
                            {message.reaction ? (
                              <BubbleReactions aria-label={`${t("reaction")}: ${message.reaction}`} align={reactionAlign}>
      ...
      <div className="px-2">
        <Tabs defaultValue="reply" className="gap-0 rounded-md border">
          ...
          <TabsContent value="reply" className="m-0">
            <MessageComposer placeholder={t("type_message")} />
          </TabsContent>
          <TabsContent value="note" className="m-0">
            <MessageComposer placeholder={t("type_note")} />
          </TabsContent>
        </Tabs>
      </div>
    </div>
    ...
    >
      <InputGroup className="border-0 bg-transparent shadow-none has-[[data-slot=input-group-control]:focus-visible]:border-0 has-[[data-slot][aria-invalid=true]]:border-0 has-[[data-slot=input-group-control]:focus-visible]:ring-0 has-[[data-slot][aria-invalid=true]]:ring-0 dark:bg-transparent dark:has-[[data-slot][aria-invalid=true]]:ring-0">
        ...
        <InputGroupAddon align="block-end">
          <InputGroupButton aria-label={t("format")} type="button" size="icon-sm">
            <Type />
          </InputGroupButton>
          <InputGroupButton aria-label={t("emoji")} type="button" size="icon-sm">
            <Smile />
          </InputGroupButton>
          <InputGroupButton aria-label={t("attach")} type="button" size="icon-sm">
            <Paperclip />
          </InputGroupButton>
          <InputGroupButton aria-label={t("insert_link")} type="button" size="icon-sm">
            <Link />
          </InputGroupButton>
          <InputGroupButton aria-label={t("ai_assist")} type="button" size="icon-sm" variant="outline">
            <Sparkles />
          </InputGroupButton>
