    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        <div className="flex items-center gap-2">
          <h1 className="font-medium text-lg tracking-tight sm:text-xl">#{shipment.id}</h1>
          <Button variant="ghost" size="icon-sm" aria-label={t("copy_id")}>
            <Copy />
          </Button>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs sm:text-sm">
          <Badge variant="outline" className={cn("gap-1.5", statusBadgeClasses[shipment.status])}>
            ...
            {t(`status.${shipment.status.replace(/\s/g, "_")}`)}
          </Badge>
          <span className="text-muted-foreground">·</span>
          <span className="text-foreground">{shipment.progress}% {t("complete")}</span>
          <span className="text-muted-foreground">·</span>
          <span className="text-foreground">
            {t("eta")}: {shipment.eta} {shipment.etaMeta}
          </span>
        </div>
      </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 text-muted-foreground">
              <span className="text-xs leading-none tracking-tight">{shipment.customer.id}</span>
      ...
      <div className="flex flex-col gap-8">
        <div className="flex items-start justify-between gap-4">
          <h2 className="font-medium">{t("cargo_details")}</h2>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-5 md:grid-cols-[1.35fr_1fr_1.1fr_1.15fr_1fr]">
          ...
          <div className="flex flex-col gap-2">
            <div className="text-muted-foreground text-xs leading-none md:text-sm">{t("total_weight")}</div>
            <div className="text-sm leading-none">{shipment.weight}</div>
          </div>

          <div className="flex flex-col gap-2">
            <div className="text-muted-foreground text-xs leading-none md:text-sm">{t("transport_mode")}</div>
          ...
          <div className="flex flex-col gap-2 md:text-right">
            ...
            <div className="text-sm leading-none">{shipment.progress}% {t("complete")}</div>
          </div>
        </div>
      </div>
        ...
        <div className="h-full min-h-0 py-2">
          <Tabs defaultValue="overview" className="h-full gap-0">
            ...
            <TabsContent className="p-4" value="documents">
              <div className="grid h-full place-items-center rounded-md border border-dashed text-muted-foreground text-sm">
                {t("coming_soon.documents")}
              </div>
            </TabsContent>
            <TabsContent className="p-4" value="activity">
              <div className="grid h-full place-items-center rounded-md border border-dashed text-muted-foreground text-sm">
                {t("coming_soon.activity")}
              </div>
            </TabsContent>
          </Tabs>
        </div>
