...
export function InvoicePaper({ invoice }: { invoice: InvoiceFormValues }) {
  ...
  const discountLabel = invoice.discountType === "percent" ? `${t("discount")} ${discountValue}%` : t("discount");
    ...
    >
      <header className="flex flex-col gap-10">
        ...
        <section className="grid grid-cols-2 gap-14 text-sm leading-relaxed">
          <div>
            <p>{t("reference")}: {invoice.referenceNumber}</p>
            <p>{t("issued")}: {invoice.issuedDate}</p>
            <p>{t("payment_due")}: {invoice.paymentDueDate}</p>
          </div>
          <div>
            <p>{t("payment_account")}</p>
            <p>{invoice.from.paymentAccountName}</p>
            <p>{t("routing_no")} {invoice.from.routingNumber}</p>
          </div>
        </section>
        ...
        <section className="grid grid-cols-2 gap-14 text-sm leading-relaxed">
          <div>
            ...
            <p>{t("tax_id")}: {invoice.from.taxId}</p>
          </div>
          <div>
            <p className="mb-4 font-semibold uppercase">{t("bill_to")}</p>
            <p>{invoice.to.name}</p>
            {invoice.to.addressLines.map((line) => (
              <p key={line}>{line}</p>
            ))}
            <p>{t("tax_id")}: {invoice.to.taxId}</p>
          </div>
        </section>
      </header>
      ...
      <div className="flex flex-col gap-5">
        <section className="text-sm">
          <div className="grid grid-cols-[1fr_74px_116px_116px] bg-stone-200 px-3 py-3 font-semibold uppercase">
            ...
            <span className="text-right">{t("unit_cost")}</span>
            <span className="text-right">{t("line_total")}</span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-14 text-sm leading-relaxed">
          <section className="col-start-2 space-y-2">
            ...
            <div className="border-current border-y-2 py-3">
              <div className="flex justify-between gap-8">
                <span className="font-semibold uppercase">{t("balance_due")}</span>
      ...
      <footer className="absolute right-12.25 bottom-11 left-12.25 grid grid-cols-2 gap-14 text-neutral-500 text-sm leading-relaxed">
        ...
        <div>
          <p>{t("prepared_for_processing")}</p>
          <p>{t("issued_by")} {invoice.from.issuerName}</p>
        </div>
      </footer>
    </article>
  );
