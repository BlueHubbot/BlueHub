...
export function Users({ users }: { users: UserRow[] }) {
  ...
  const workspaceFilter = (table.getColumn("workspace")?.getFilterValue() as string | undefined) ?? filters.workspace[0];
      ...
      <CardContent className="flex flex-col gap-4 px-0">
        <div className="flex flex-wrap items-center justify-between gap-3 px-4">
                ...
                <SelectGroup>
                  {filters.role.map((option) => (
                    <SelectItem key={option} value={option}>
                      {option === "All" ? t("filters.all") : option}
                    </SelectItem>
                  ))}
                </SelectGroup>
              </SelectContent>
          ...
          <Select value={workspaceFilter} onValueChange={(value) => setColumnSelectFilter("workspace", value)}>
            <SelectTrigger size="sm">
              <span className="text-muted-foreground">{t("filters.workspace")}:</span>
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="flex items-center justify-between gap-3 px-4">
          <div className="text-muted-foreground text-sm">{selectedCount} {t("selected")}</div>

          <Tabs defaultValue="list">
            <TabsList>
              <TabsTrigger value="list" aria-label={t("list_view")}>
                <Rows3 />
              </TabsTrigger>
              <TabsTrigger value="grid" aria-label={t("grid_view")}>
                <Grid />
              </TabsTrigger>
            </TabsList>
          </Tabs>
