...
const qualitySeries = [
  ...
  { date: "2026-04-14T00:00:00.000Z", absoluteQuality: 1.2, baselineQuality: 4.1 },
...
const chartConfig = {
  actualQuality: {
    color: "var(--chart-3)",
    label: "کیفیت واقعی ترافیک",
  },
  baselineQuality: {
    color: "var(--muted-foreground)",
    label: "خط مبنا (شاخص اولیه)",
  },
...
function formatWeek(value: number) {
  ...
  return weekIndex >= 0 ? `هفته ${weekIndex + 1}` : "";
}
      ...
      <CardContent>
        <ChartContainer config={chartConfig} className="h-68 w-full">
          <ComposedChart data={chartData} margin={{ bottom: 0, left: 0, right: 0, top: 0 }}>
            ...
            <ChartTooltip
              cursor={false}
              content={<ChartTooltipContent className="w-48 text-right" labelFormatter={() => t("tooltip_label")} />}
            />
            <Line
