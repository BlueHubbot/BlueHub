...
export function InsightCards() {
  const t = useTranslations("Legacy.CRM.insight");
  ...
  return (
    <div className="grid grid-cols-1 gap-4 *:data-[slot=card]:shadow-xs sm:grid-cols-2 xl:grid-cols-5">
          ...
          <ul className="flex flex-col gap-3">
            {leadsBySourceChartData.map((item) => (
              <li key={item.source} className="flex w-36 items-center justify-between">
                ...
                <span className="text-xs">{item.leads}</span>
              </li>
            ))}
          </ul>
        </CardContent>
      ...
      <Card className="col-span-1 xl:col-span-3">
        <CardHeader>
          <CardTitle>{t("project_revenue")}</CardTitle>
        </CardHeader>
        ...
        <CardFooter>
          <p className="text-muted-foreground text-xs">{t("project_footer")}</p>
        </CardFooter>
      </Card>
    </div>
  );
