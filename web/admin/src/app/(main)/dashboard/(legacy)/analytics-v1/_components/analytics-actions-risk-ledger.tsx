...
export function ActionsRiskLedger() {
  const t = useTranslations("Legacy.Analytics.ledger");
  ...
  const ledgerColumns: ColumnDef<LedgerRow>[] = [
    ...
    },
    {
      ...
      header: t("columns.idle"),
      cell: ({ row }) => <span className="text-xs">{row.original.idleDays}d</span>,
    },
    {
      accessorKey: "closeVariance",
      header: t("columns.close_variance"),
      cell: ({ row }) => <span className="text-xs">{row.original.closeVariance}</span>,
    },
    {
      accessorKey: "nextAction",
      header: t("columns.next_action"),
      cell: ({ row }) => (
        <div className="flex max-w-64 flex-col gap-1 whitespace-normal">
          {row.original.priority ? (
            <Badge variant="outline" className={cn("text-[10px] uppercase", priorityTone[row.original.priority])}>
              {t(`priority.${row.original.priority.toLowerCase()}`)}
            </Badge>
          ) : null}
      ...
      cell: ({ row }) => (
        <div className="flex justify-end">
          <Badge
            variant="outline"
            className={cn(
              "min-w-12 justify-center font-medium",
    ...
    <Card className="min-w-0 shadow-xs">
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-3 rounded-lg border bg-muted/20 p-3 text-sm sm:grid-cols-4 sm:divide-x sm:divide-border/60">
          <LedgerStat label={t("stats.critical")} value="1" detail={t("stats.critical_detail")} />
          <LedgerStat label={t("stats.escalations")} value="1" detail={t("stats.escalations_detail")} />
          <LedgerStat label={t("stats.median_inactivity")} value="31d" detail={t("stats.median_inactivity_detail")} />
          <LedgerStat
            label={t("stats.overdue_revenue")}
            value={formatCurrency(1084000, { noDecimals: true })}
            detail={t("stats.overdue_revenue_detail")}
          />
        </div>
