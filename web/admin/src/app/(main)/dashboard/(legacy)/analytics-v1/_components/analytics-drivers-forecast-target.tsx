...
export function DriversForecastTarget() {
  const t = useTranslations("Legacy.Analytics.forecast");
    ...
    <Card className="shadow-xs">
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <MetricChip label={t("metrics.attainment")} value="72.4%" note={t("metrics.attainment_note")} />
          <MetricChip label={t("metrics.weighted_pipeline")} value="$1,284,000" note={t("metrics.weighted_pipeline_note")} />
          <MetricChip label={t("metrics.forecast_confidence")} value="81.0%" note={t("metrics.forecast_confidence_note")} />
        </div>
