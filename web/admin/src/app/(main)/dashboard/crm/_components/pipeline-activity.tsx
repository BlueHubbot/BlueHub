...
export function PipelineActivity() {
  ...
  const axisMonthFormatter = React.useMemo(() => new Intl.DateTimeFormat(locale, { month: "short" }), [locale]);
  const tooltipMonthFormatter = React.useMemo(() => new Intl.DateTimeFormat(locale, { month: "short", year: "2-digit" }), [locale]);
  ...
  return (
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-12" dir="rtl">
      <Card className="xl:col-span-12">
        <CardHeader className="text-right">
          <CardTitle className="font-bold">{t('qualified_lead_flow')}</CardTitle>
            ...
            <Select defaultValue="last-12-months">
              ...
              <SelectContent>
                <SelectGroup>
                  ...
                  <SelectItem value="last-quarter">{t('last_quarter')}</SelectItem>
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
            ...
            <div className="flex flex-col gap-5 rounded-lg p-4 lg:col-span-4 text-right">
              <div className="flex flex-col gap-1">
                <div className="font-medium text-4xl  leading-none">
                  {totalQualified.toLocaleString(locale)} <span className="font-normal text-lg text-muted-foreground">{t('leads_count', { count: '' })}</span>
                </div>
                <p className="text-muted-foreground text-sm">{t('total_qualified_description')}</p>
              </div>

              <div className="flex flex-col gap-3 rounded-lg border border-border/60 p-3">
                <div className="text-[11px] text-muted-foreground uppercase tracking-widest font-bold">
                  {t('discovery_calls_booked')}
                </div>

                <div className="flex flex-col gap-1.5">
                  <div className="font-medium text-2xl  leading-none">
                    {discoveryCallsBooked.toLocaleString(locale)} <span className="font-normal text-muted-foreground text-sm">{t('meetings_count', { count: '' })}</span>
                  </div>
                  <p className="text-muted-foreground text-sm">
                    {t('discovery_progress_description', { progress: discoveryProgress.toLocaleString(locale) })}
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-2 pt-0.5">
                  ...
                  <div className="flex items-center justify-between text-xs flex-row-reverse">
                    <div className="font-medium ">{t('booked_count', { count: discoveryCallsBooked.toLocaleString(locale) })}</div>
                    <div className="text-muted-foreground ">{t('qualified_count', { count: totalQualified.toLocaleString(locale) })}</div>
                  </div>
                </div>
              </div>
            </div>
