...
function TrafficSourceIconLabel({ height, index, width, x, y }: IconLabelProps) {
  ...
  if (!source || Number.isNaN(xValue) || Number.isNaN(yValue) || Number.isNaN(widthValue) || Number.isNaN(heightValue)) {
    return null;
  }
...
function TrafficSourceNameLabel({ height, index, x, y }: SourceLabelProps) {
  ...
  if (!source || Number.isNaN(xValue) || Number.isNaN(yValue) || Number.isNaN(heightValue)) return null;
...
function TrafficSourceChangeLabel({ height, value, y }: SourceChangeLabelProps) {
  ...
  if (typeof value !== "string" || Number.isNaN(yValue) || Number.isNaN(heightValue)) return null;
...
const trafficSources = [
  { name: "Meta", visits: "5,640", share: 38, change: "+18%", icon: siMeta },
  { name: "Google", visits: "3,740", share: 25, change: "-6%", icon: siGoogle },
  { name: "Shopify", visits: "2,960", share: 20, change: "+7%", icon: siShopify },
  { name: "TikTok", visits: "1,340", share: 10, change: "+9%", icon: siTiktok },
  { name: "eBay", visits: "1,080", share: 7, change: "-3%", icon: siEbay },
] as const;

export function TrafficSources() {
  const t = useTranslations("Ecommerce.traffic_sources");
