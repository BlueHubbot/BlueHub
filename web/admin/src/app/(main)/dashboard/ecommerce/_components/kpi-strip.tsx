...
export function KpiStrip() {
  const t = useTranslations("Ecommerce.kpis");

  const kpiItems = [
    { key: "total_sales", value: "$48,560.00", change: "+15.8%", trend: "up", vs: t("vs_week"), icon: DollarSign },
    { key: "total_orders", value: "379", change: "+8.3%", trend: "up", vs: t("vs_week"), icon: ShoppingBag },
    { key: "customer_growth", value: "820", change: "+12.5%", trend: "up", vs: t("vs_month"), icon: Users },
    { key: "average_order", value: "$128", change: "-$4.20", trend: "down", vs: t("vs_week"), icon: ReceiptText },
    { key: "return_requests", value: "18", change: "+0.6%", trend: "down", vs: t("vs_month"), icon: RotateCcw },
    { key: "stock_accuracy", value: "97%", change: "+2.4 pts", trend: "up", vs: t("vs_audit"), icon: PackageCheck },
  ];
      ...
      <div>
        <div className="grid grid-cols-1 xl:grid-cols-12">
          <div className="grid grid-cols-1 md:grid-cols-2 md:grid-rows-3 xl:col-span-5 xl:border-r">
            {kpiItems.map((item, index) => (
              <Card 
                key={item.key} 
                className={`
                  h-full rounded-none border-0 ring-0 
                  ${index % 2 === 0 ? 'md:border-r' : ''} 
                  ${index < 4 ? 'border-border border-b' : ''}
                  ${index === 4 ? 'md:border-b-0' : ''}
                `}
              >
                <CardHeader>
                  <CardTitle className="font-normal text-sm">{t(`${item.key}.title`)}</CardTitle>
                  <CardDescription className="text-3xl text-foreground leading-none tracking-tight">
                    {item.value}
                  </CardDescription>
                  <CardAction className="grid size-6 place-items-center rounded-sm bg-muted">
                    <item.icon className="size-3 text-foreground" />
                  </CardAction>
                </CardHeader>
                <CardContent>
                  <div className="text-sm">
                    <span className={item.trend === "up" ? "text-green-700 dark:text-green-300" : "text-destructive"}>
                      {item.change}
                    </span>
          ...
          <Card className="h-full rounded-none border-0 ring-0 xl:col-span-7">
            <CardHeader>
              <CardTitle className="font-normal">{t("sales_overview")}</CardTitle>
              <CardAction>
