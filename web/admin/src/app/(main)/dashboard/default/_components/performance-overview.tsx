...
const chartValues = [
  ...
  { newCustomers: 8715, activeAccounts: 6205, trackingUsers: 4745 },
...
export function PerformanceOverview() {
  ...
  // مقداردهی داینامیک کانفیگ چارت با استفاده از کلیدهای ترجمه درون کامپوننت
  const chartConfig = {
    newCustomers: {
      label: t("new_customers_label"),
      color: "var(--chart-1)",
    },
    activeAccounts: {
      label: t("active_accounts_label"),
      color: "var(--chart-2)",
    },
    returningUsers: {
      label: t("returning_users_label"),
      color: "var(--chart-3)",
    },
  ...
  return (
    <Card className="@container/card">
      <CardHeader>
        <CardTitle className="leading-none">{t("customer_activity")}</CardTitle>
        <CardDescription>
          <span className="@[540px]/card:block hidden">{t("customer_activity_desc_long")}</span>
          <span className="@[540px]/card:hidden">{t("customer_activity_desc_short")}</span>
        </CardDescription>
        <CardAction className="flex items-center gap-2">
          <Select defaultValue="quarter">
            <SelectTrigger size="sm" className="w-28">
              <SelectValue placeholder={t("three_months")} />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
                <SelectLabel>{t("period")}</SelectLabel>
                <SelectItem value="quarter">{t("three_months")}</SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>
          ...
          <Select defaultValue="all">
            <SelectTrigger size="sm" className="w-32">
              <SelectValue placeholder={t("all_segments")} />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
                <SelectLabel>{t("segments")}</SelectLabel>
                <SelectItem value="all">{t("all_segments")}</SelectItem>
        ...
        <ChartContainer config={chartConfig} className="aspect-auto h-80 w-full">
          <ComposedChart data={chartData} margin={{ top: 0 }}>
            ...
            <XAxis
              ...
              minTickGap={48}
              tickFormatter={(value) => {
                // هماهنگ‌سازی نمایش فرمت ماه و روز بر اساس لوکال جاری پلتفرم
                return parseISO(value).toLocaleDateString(currentLocale === "fa" ? "fa-IR" : "en-US", {
                  month: "short",
                  day: "numeric",
                });
              }}
              ...
              content={
                <ChartTooltipContent
                  ...
                  labelFormatter={(value) => {
                    // نمایش کامل تاریخ در تولتیپ متناسب با سیستم لوکالیزیشن
                    return parseISO(value).toLocaleDateString(currentLocale === "fa" ? "fa-IR" : "en-US", {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
