        ...
        <div className="min-w-0 space-y-1">
          <div className="flex items-center gap-0.5">
            <Button
              ...
              aria-label={t("drag_column", { title: column.title })}
              {...attributes}
              {...listeners}
            >
              <GripVertical />
          ...
          <p className="text-muted-foreground text-sm leading-none">
            {tasks.length} {tasks.length === 1 ? t("task_singular") : t("task_plural")}
          </p>
        </div>
        <div className="-mr-2 flex items-center gap-0.5 text-muted-foreground">
          <Button variant="ghost" size="icon-sm" aria-label={t("add_task_to_column", { title: column.title })}>
            <Plus />
          </Button>
          <Button variant="ghost" size="icon-sm" aria-label={t("column_actions", { title: column.title })}>
            <MoreVertical />
          </Button>
        </div>
      </div>
