        ...
        {groups.map((group) => (
          <section key={group.id} className="flex flex-col gap-1.5">
            <div className="mx-3 text-muted-foreground text-xs">
              {t(`groups.${group.id}`)} ({group.items.length})
            </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        >
                          {formatMailDate(item.receivedAt, dateLocale)}
                        </div>
                      </div>
...
function formatMailDate(date: string, locale: typeof faIR) {
  const mailDate = new Date(date);

  if (differenceInDays(new Date(), mailDate) <= 3) {
    return formatDistanceToNow(mailDate, { addSuffix: true, locale });
  }

  return format(mailDate, "d MMM yyyy", { locale });
}