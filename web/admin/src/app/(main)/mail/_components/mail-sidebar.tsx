    ...
    <Sidebar collapsible="icon" className="absolute inset-y-0 h-full **:data-[sidebar=sidebar]:bg-background">
      <SidebarHeader className="gap-3 py-3 pb-1">
        <div className="flex items-center justify-between">
          {isCollapsed ? (
            ...
            <>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon-sm" aria-label={t("open_account_menu")}>
                    <EllipsisVertical />
                  </Button>
                </DropdownMenuTrigger>
        ...
        <Button size={isCollapsed ? "icon-sm" : "sm"} variant="outline" className="group-data-[state=expanded]:w-full">
          ...
          <span className="group-data-[state=collapsed]:hidden">{t("new_email")}</span>
        </Button>
      </SidebarHeader>
      <SidebarContent>
        <SidebarGroup>
        ...
        <SidebarGroup>
          <SidebarGroupLabel className="font-normal">{t("folders")}</SidebarGroupLabel>
...
function renderNavItem(nav: MailNavItem, t: (key: string) => string) {
  return (
    <SidebarMenuItem key={nav.id}>
      <SidebarMenuButton className="[&_svg]:size-3.5" size="sm" isActive={nav.isActive} tooltip={t(`nav.${nav.id}`)}>
        <nav.icon />
        <span className="font-medium">{t(`nav.${nav.id}`)}</span>
      </SidebarMenuButton>
