    ...
    <div className="flex h-full min-h-0 flex-col gap-3 px-2 py-3">
      <div className="flex items-center">
        <div className="flex items-center gap-2">
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon-sm" aria-label={t("close_message")} onClick={handleClose}>
                <X />
              </Button>
            </TooltipTrigger>
            <TooltipContent>{t("close_message")}</TooltipContent>
          </Tooltip>
          <Separator className="h-4 data-vertical:self-center" orientation="vertical" />
          <div className="flex items-center gap-0">
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon-sm" aria-label={t("previous_message")}>
                  <ChevronLeft />
                </Button>
              </TooltipTrigger>
              <TooltipContent>{t("previous_message")}</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon-sm" aria-label={t("next_message")}>
                  <ChevronRight />
                </Button>
              </TooltipTrigger>
              <TooltipContent>{t("next_message")}</TooltipContent>
            </Tooltip>
          </div>
        </div>
        ...
        <div className="ml-auto flex items-center gap-2">
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon-sm" aria-label={t("pin_thread")}>
                <Pin />
              </Button>
            </TooltipTrigger>
            <TooltipContent>{t("pin_thread")}</TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon-sm" aria-label={t("reply")}>
                <Reply />
              </Button>
            </TooltipTrigger>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon-sm" aria-label={t("move_trash")}>
                <Trash2 className="text-destructive" />
              </Button>
            </TooltipTrigger>
            <TooltipContent>{t("move_trash")}</TooltipContent>
          </Tooltip>
        </div>
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        {mail ? (
          <div className="flex min-h-0 flex-1 flex-col gap-3">
              ...
              <div className="flex h-full flex-col gap-1">
                ...
                <div className="flex items-center gap-2">
                  <div className="text-muted-foreground text-xs">
                    {t("to")}: <span className="text-foreground">{mail.to.map((recipient) => recipient.name).join(", ")}</span>
                  </div>
            ...
            <div className="mt-auto flex flex-col gap-3">
              ...
              <InputGroup>
                ...
                <InputGroupInput className="text-xs" placeholder={t("reply_placeholder", { name: mail.from.name })} />
          ...
          <div className="grid h-full place-items-center text-muted-foreground text-sm">{t("no_email_selected")}</div>
        )}
      </div>
    </div>
  );
