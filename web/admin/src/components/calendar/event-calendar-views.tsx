...
// 💥 تغییر کلیدی: اضافه شدن یونیکد اعداد فارسی به ریجکس
const toEnNums = (str: string | number | undefined | null) => {
  if (str === null || str === undefined) return '';
  return String(str).replace(/[۰-۹٠-٩]/g, d => (d.charCodeAt(0) & 15).toString());
};
...
const dayRowCommonClasses: CalendarOptions = {
  ...
  listItemEventTimeClass: (info) => cn(
    ...
    'order-1 whitespace-nowrap overflow-hidden shrink-1 font-sans',
  ),
...
export function EventCalendarViews({
  ...
  return (
    <FullCalendar
      ...
      popoverFormat={{ year: 'numeric', month: 'long', day: 'numeric' }}

      /* رندرهای سراسری محتوا برای انگلیسی کردن قطعی تمام متون تقویم */
/* دور زدن تایپ‌اسکریپت برای پراپ‌های اختصاصی پلاگین‌ها */
      {...({
        eventContent: (info: any) => (
          <div className="flex items-center gap-1 overflow-hidden px-1 w-full justify-between">
            {info.timeText && <span className="font-sans text-xs opacity-80">{toEnNums(info.timeText)}</span>}
            <span className="font-medium overflow-hidden text-ellipsis whitespace-nowrap">{info.event.title}</span>
          </div>
        ),
        slotLabelContent: (info: any) => (
          <span className="font-sans text-xs text-muted-foreground">{toEnNums(info.text)}</span>
        ),
        allDayContent: (info: any) => (
          <span className="font-sans text-xs">{toEnNums(info.text)}</span>
        ),
        moreLinkContent: (info: any) => (
          <span className="font-sans text-xs">{toEnNums(info.text)}</span>
        )
      } as any)}
      ...
      /* List-Item Event */
      listItemEventTimeClass='text-muted-foreground font-sans'
      ...
      blockEventTimeClass='whitespace-nowrap overflow-hidden shrink-1 font-sans'
      ...
      dayCellTopClass={(info) => cn(
        ...
        'flex flex-row justify-end font-sans',
      )}
